...
DQI scores do not gate the results. The three human-in-the-loop gates that pause Cortex — coverage below 80%, proxy substitution required, cross-database spread above 2× — are separate from DQI. A candidate can pass all three gates and still carry a low temporal score. That is intentional: gating on DQI would suppress legitimate datasets that are geographically or temporally imperfect but still the best available. Cortex surfaces the information; the practitioner carries the judgment.
...
To see how Cortex's three HITL gates interact with DQI in the broader matching workflow, read the [Methodology](/methodology) page. To run a search and see DQI scores on live candidates, open [Cortex Chat](/product/chat).
