...
## What Cortex actually does with the scores

The five dimensions matter to your verifier. They should not be your job to interpret one by one.

When you search a material, Cortex reads the scores on every candidate and gives you the answer in the form a working LCA practitioner needs:

- *Can this row be used for my BOM?*
- *Of the candidates Cortex returned, which is the best fit for my situation?*
- *Why this one, and why not that one?*

A worked example. You search "304 stainless coil, Chinese mill." Two candidates come back: an Ecoinvent EU-average dataset, and HiQLCD's North China BF-BOF entry. The Ecoinvent record has high reliability — well-sourced, well-documented — but the geographic score is low for your context. The HiQLCD record has the right geography and the right process route, with a slightly older reference year.

Cortex tells you: *the HiQLCD candidate is the better fit for a Chinese-mill BOM. The geographic and technology accuracy outweighs the smaller temporal gap. The choice is defensible to a verifier — the proxy notes can document the reasoning.*

The scores are still right there if your verifier wants to see them. You did not have to read all five and infer the answer.

## When the scores tell you not to use a row

A low Temporal score on every candidate means the database has not been updated for your time window. A low Geographic score with no better candidate means a proxy decision is the only option — and Cortex will say so explicitly, with the bias direction named, before that row enters the export.

Cortex does not silently filter low-quality candidates out of the result list. Filtering would suppress legitimate datasets that are imperfect but the best available for an unusual material. Instead the candidates appear with the scores attached, and Cortex calls the choice — leaving the practitioner the final word.
